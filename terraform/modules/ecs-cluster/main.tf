...
#                    the image and resolves `secrets` from Secrets Manager. The
#                    container never holds these permissions.
...
#
# Injected secrets are resolved by the execution role, which is precisely why the
# running container cannot read the secret store itself.
...
# Scoped to the specific secrets these tasks use — not secretsmanager:* on "*".
# The managed policy above covers ECR and logs but deliberately not secrets.
